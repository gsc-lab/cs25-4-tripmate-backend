<?php
    ...
    return function (\AltoRouter $router, Request $request, Response $response): void {
        // 회원가입 라우팅 등록
        $router->map('POST', '/api/v1/users', [AuthController::class, 'register']);

        // 로그인 라우팅 등록
        $router->map('POST', '/api/v1/auth/login', [AuthController::class, 'login']);

        // 로그아웃 라우팅 등록
        $router->map('POST', '/api/v1/auth/logout', [AuthController::class, 'logout']);
    };